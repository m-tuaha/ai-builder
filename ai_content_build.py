...
#--- PIL Helper: for color swap, inlcudes _rgb_to_hex, _avg_hex_in_regions & color_swap ----
def _rgb_to_hex(rgb):
    r, g, b = rgb[:3]
    return f"#{int(r):02X}{int(g):02X}{int(b):02X}"
    
def _avg_hex_in_region(img_bytes: bytes, bbox: tuple[int,int,int,int] | None) -> str:
    """
    Fast-ish average color. If bbox is None, uses whole image.
    """
    im = Image.open(io.BytesIO(img_bytes)).convert("RGB")
    if bbox:
        im = im.crop(bbox)
    # Downscale to reduce pixels, then average
    im = im.resize((32, 32))
...
def color_swap_image(
    img_bytes: bytes,
    src_hex: str,
    dst_hex: str,
    tolerance_deg: int = 25,      # hue window (±°) for HSV match
    strength: float = 0.85,       # hue push 0..1
    region_bbox: tuple[int,int,int,int] | None = None,
    sat_boost: float = 0.60,      # saturation push (helps gray/white fabric)
    val_floor: float = 0.04,      # ignore only ultra-dark
    rgb_tol_pct: int = 12,        # fallback RGB distance % (0..100) for whites/grays/blacks
) -> bytes:
    """
    Recolor pixels similar to src_hex toward dst_hex.
    - Primary: HSV hue match within tolerance_deg.
    - Fallback: RGB Euclidean distance for near-white/near-black/gray sources.
    Preserves brightness (V) and pushes saturation for believable recolor.
    """
    base = Image.open(io.BytesIO(img_bytes)).convert("RGBA")
    work_box = region_bbox or (0, 0, base.width, base.height)

    # parse colors
    def _hex_to_hsv(hx: str):
        r, g, b = _hex_to_rgb(hx)
        return colorsys.rgb_to_hsv(r/255.0, g/255.0, b/255.0)

    src_h, src_s, src_v = _hex_to_hsv(src_hex)
    dst_h, dst_s, _ = _hex_to_hsv(dst_hex)

    tol_h = max(0.0, min(180.0, float(tolerance_deg))) / 360.0
    k_h = max(0.0, min(1.0, float(strength)))
    k_s = max(0.0, min(1.0, float(sat_boost)))
    # RGB fallback threshold: map 0..100% to 0..441.67 (max RGB distance)
    rgb_thresh = max(0, min(100, int(rgb_tol_pct))) * 4.4167

    # heuristics: when to use RGB fallback (source is near white/black/gray)
    use_rgb_fallback = (src_s < 0.12) or (src_v > 0.92) or (src_v < 0.10)

    layer = base.crop(work_box).convert("RGBA")
    rgb = layer.convert("RGB")
    a = layer.split()[-1]

    px = list(rgb.getdata())
    new_px = []
    # precompute source RGB for distance
    ...
    for (r, g, b) in px:
        ...
        matched = False
        if use_rgb_fallback:
            # RGB distance match (helps text/white/black/gray)
            dr = r - sr
            dg = g - sg
            db = b - sb
            dist = (dr*dr + dg*dg + db*db) ** 0.5
            matched = dist <= rgb_thresh
        else:
            # HSV hue distance match (usual case)
            dh = abs(h - src_h)
            dh = min(dh, 1.0 - dh)
            matched = (dh <= tol_h)

        if matched:
            # push hue toward target
            h_new = (1.0 - k_h) * h + k_h * dst_h
            # push saturation toward target to recolor flats
            s_new = (1.0 - k_s) * s + k_s * max(s, dst_s)
            s_new = max(0.0, min(1.0, s_new))
            r2, g2, b2 = colorsys.hsv_to_rgb(h_new, s_new, v)
            new_px.append((int(r2*255), int(g2*255), int(b2*255)))
        else:
            new_px.append((r, g, b))

    out_rgb = Image.new("RGB", rgb.size)
    out_rgb.putdata(new_px)
    out_rgba = Image.merge("RGBA", (*out_rgb.split(), a))

    out_img = base.copy()
    out_img.paste(out_rgba, work_box)
...
with tab2:
    ...
    # ---------- QUICK ACTIONS (Pillow) ----------
        ...
        qa = st.selectbox("Choose an action", ["Add Text Overlay", "Resize/Crop", "Color Swap"], key="qa_choice")
        ...
        # Color Swap mode when selected
        if qa == "Color Swap":
            if not src_bytes:
                st.info("Select or upload an image first to enable color swap.")
            else:
                st.markdown("##### Pick area and colors")

                # --- Optional region crop (freeform) ---
                use_region = st.checkbox("Limit to a selected region", value=False, key="cs_use_region")
                bbox = None
                if use_region:
                    pil_src = Image.open(io.BytesIO(src_bytes)).convert("RGB")
                    st.caption("Drag the rectangle; live crop preview below.")

                    # Single cropper returning BOTH image and box (avoid duplicate widgets)
                    both = st_cropper(
                        pil_src,
                        realtime_update=True,
                        box_color="#00483B",   # your teal
                        aspect_ratio=None,
                        return_type="both",
                        key="cs_cropper_both"
                    )

                    # Unpack outputs (supports tuple(image, box) or just box dict)
                    if isinstance(both, tuple) and len(both) == 2:
                        cropped_img, crop_box = both
                    else:
                        cropped_img, crop_box = None, both

                    # Parse crop_box robustly (dict with left/top/width/height or right/bottom)
                    if isinstance(crop_box, dict):
                        left = int(crop_box.get("left", 0))
                        top = int(crop_box.get("top", 0))
                        if "width" in crop_box and "height" in crop_box:
                            right = left + int(crop_box["width"])
                            bottom = top + int(crop_box["height"])
                        else:
                            right = int(crop_box.get("right", left))
                            bottom = int(crop_box.get("bottom", top))
                        # clamp to image bounds
                        left = max(0, min(left, pil_src.width))
                        top = max(0, min(top, pil_src.height))
                        right = max(left, min(right, pil_src.width))
                        bottom = max(top, min(bottom, pil_src.height))
                        bbox = (left, top, right, bottom)

                    st.markdown("**Region preview:**")
                    if cropped_img is not None:
                        st.image(cropped_img, use_container_width=True)
                    elif bbox:
                        st.image(pil_src.crop(bbox), use_container_width=True)

                # --- Color controls ---
                colc1, colc2 = st.columns(2)
                if "cs_src_hex_pending" in st.session_state:
                    st.session_state["cs_src_hex"] = st.session_state.pop("cs_src_hex_pending")
    
                with colc1:                        
                    src_hex = st.color_picker("Source (color to replace)", value="#FFFFFF", key="cs_src_hex")

                    if use_region and st.button("Sample from region", key="cs_sample"):
                        try:
                            sampled = _avg_hex_in_region(src_bytes, bbox)
                            st.session_state["cs_src_hex"] = sampled
                            src_hex = sampled
                            st.success(f"Sampled source color: {sampled}")
                        except Exception as _e:
                            st.warning(f"Sampling failed: {_e}")

                    # --- Eyedropper: click to set Source color ---
                    with st.expander("🎯 Click to pick source color", expanded=False):
                        pick_img = Image.open(io.BytesIO(src_bytes)).convert("RGB")
                        if use_region and bbox:
                            pick_img = pick_img.crop(bbox)

                        click = streamlit_image_coordinates(pick_img, key="cs_eyedrop")
                        if click is not None:
                            x, y = int(click["x"]), int(click["y"])
                            x = max(0, min(x, pick_img.width - 1))
                            y = max(0, min(y, pick_img.height - 1))
                            r, g, b = pick_img.getpixel((x, y))[:3]
                            picked_hex = f"#{r:02X}{g:02X}{b:02X}"
                            st.session_state["cs_src_hex"] = picked_hex
                            src_hex = picked_hex
                            st.success(f"Picked {picked_hex} at ({x},{y})")
                            st.session_state["cs_src_hex_pending"] = picked_hex
                            st.rerun()
                
                with colc2:
                    dst_hex = st.color_picker("Target color", value="#FFFF00", key="cs_dst_hex")

                trow1, trow2, trow3 = st.columns(3)
                with trow1:
                    tolerance = st.slider("Tolerance (hue ±°)", 5, 60, 25, step=1, key="cs_tol")
                with trow2:
                    strength = st.slider("Strength", 0.10, 1.00, 0.85, step=0.05, key="cs_strength")
                with trow3:
                    sat_boost = st.slider("Saturation boost", 0.00, 1.00, 0.60, step=0.05, key="cs_sat_boost")

                rgb_tol_pct = st.slider("RGB tolerance (fallback for whites/grays/blacks) %", 0, 100, 12, step=1, key="cs_rgbtol")

                # ---- Live preview ----
                try:
                    preview_cs = color_swap_image(
                        img_bytes=src_bytes,
                        src_hex=src_hex,
                        dst_hex=dst_hex,
                        tolerance_deg=int(tolerance),
                        strength=float(strength),
                        region_bbox=bbox if use_region else None,
                        sat_boost=float(sat_boost),
                        rgb_tol_pct=int(rgb_tol_pct),
                    )
                    st.markdown("**Live Preview:**")
                    st.image(preview_cs, use_container_width=True)
                except Exception as _pe:
                    st.warning(f"Preview failed: {_pe}")

                # ---- Apply & log ----
                if st.button("✅ Apply Color Swap", use_container_width=True, key="apply_color_swap"):
                    # quota (same counter you already use)
                    try:
                        used = count_success_images_today(st.session_state.auth_user["id"])
                        if used >= 10:
                            st.warning("Daily image quota reached (10). Ask admin to reset or try tomorrow.")
                    ...
                    try:
                        ...
                        # Save + log (mode=color_swap, model=pillow)
                        uid = st.session_state.auth_user["id"]
                        prompt_for_log = f"Color swap {src_hex} → {dst_hex}"
                        img_id, img_url = save_image_return_url(
                            uid, out_bytes, mode="color_swap", model="pillow", prompt=prompt_for_log
                        )
                        if img_url:
                            st.session_state["last_image_url"] = img_url
