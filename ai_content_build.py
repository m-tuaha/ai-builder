...
st.session_state.setdefault("ph_last_params", None)
...
with tab3:
    ...
    existing_variants = st.session_state.get("ph_variants", [])
    if (
        st.session_state.get("ph_selected_idx") is not None
        and (st.session_state.ph_selected_idx >= len(existing_variants))
    ):
...
Photographic, crisp details, commercial polish, square 1:1, social-media ready."""
                ...
                try:
                    desired_variants = st.session_state.get("ph_variant_target", 2)
                    try:
                        desired_variants = int(desired_variants)
                    except Exception:
                        desired_variants = 2
                    desired_variants = max(1, min(2, desired_variants))
                    engine_choice = st.session_state.get("ph_engine", "Model B")
                    buffer: list[dict] = []
                    with st.spinner("Generating hero variants..."):
                    ...
                    st.session_state.ph_variants = buffer[:2]
                    st.session_state.ph_selected_idx = None
                    st.session_state.ph_last_params = {
                        ...
                        "aspect_ratio": (
                            "1:1"
                            if engine_choice == "Model A"
                            else st.session_state.get("ph_ar", "match_input_image")
                        ),
                        "output_format": (
                            "png"
                            if engine_choice == "Model A"
                            else st.session_state.get("ph_outfmt", "jpg")
                        ),
                    }
                    if st.session_state.ph_variants:
                        st.success("Hero variants ready!")
                        ph_log(
                            uid,
                            "image_generate_done",
                            mode="product_hero",
                            ...
                            variant_count=len(st.session_state.ph_variants),
                        )
                except Exception as e:
                    st.error(f"❌ {e}")
    ...
    with center_col:
        ...
        ph_variants = st.session_state.get("ph_variants", [])
        if not ph_variants:
            st.caption("Your generated variants will appear here.")
        else:
            col_a, col_b = st.columns(2)
            for idx, variant in enumerate(ph_variants[:2]):
                target_col = col_a if idx == 0 else col_b
                with target_col:
                    st.image(variant["img"], use_container_width=True)
                    ...
                    if st.session_state.get("ph_selected_idx") == idx:
                        st.caption("Selected")

            if len(ph_variants) == 1:
                with col_b:
                    st.markdown(
                        "<div style='border:1px dashed rgba(0,0,0,0.2); border-radius:8px; padding:20px; text-align:center;'>",
                        unsafe_allow_html=True,
                    )
                    if st.button("+ Add Variant", key="ph_add_variant", use_container_width=True):
                        last_params = st.session_state.get("ph_last_params")
                        if not last_params:
                            st.warning("Generate a hero first.")
                        else:
                            try:
                                with st.spinner("Generating another variant..."):
                                    if last_params.get("engine") == "Model A":
                                        seed_val = random.randint(100000, 999999)
                                        new_bytes = generate_kontext_max(
                                            last_params.get("prompt", ""),
                                            last_params.get("data_url", ""),
                                            aspect_ratio="1:1",
                                            seed=seed_val,
                                            output_format="png",
                                        )
                                        ...
                                        new_bytes = generate_nano_banana_replicate(
                                            prompt=last_params.get("prompt", ""),
                                            image_data_urls=[last_params.get("data_url", "")],
                                            aspect_ratio=last_params.get(
                                                "aspect_ratio", "match_input_image"
                                            ),
                                            output_format=last_params.get(
                                                "output_format", "jpg"
                                            ),
                                        )
                                        new_variant = {
                                            ...
                                            "seed": None,
                                            "engine": "B",
                                            "format": last_params.get(
                                                "output_format", "jpg"
                                            ),
                                        }
                                st.session_state.ph_variants.append(new_variant)
                                st.session_state.ph_selected_idx = len(
                                    st.session_state.ph_variants
                                ) - 1
                                ph_log(
                                    uid,
                                    "image_generate_done",
    ...
    with right_col:
        ...
        if selected_idx is None or selected_idx >= len(ph_variants):
            st.caption("Select a variant to enable actions")
        else:
            variant = ph_variants[selected_idx]
            edit_options = {
                "Relevant Props": "Add 2–3 small props matching the product’s use; realistic scale; balanced composition.",
                "Packaging": "Place branded packaging behind the product, slightly out of focus; matching colors.",
                "Hang Tag (blank)": "Add a small premium hang tag near the product; leave blank for later text.",
                "Liquid Splash": "Subtle dynamic splash behind the product; do not cover the label.",
                "Levitating": "Slightly levitating look with soft rim light; keep a believable contact shadow.",
                "Surface/Shadow Polish": "Matte studio surface; refined soft shadow; avoid harsh reflections.",
                "Lifestyle": "Place in a simple lifestyle setting (kitchen/bathroom/desk); shallow depth of field; natural light.",
                "Flat-lay": "Top-down flat-lay on a clean backdrop with up to 3 minimal props.",
...
Photographic, crisp details, commercial polish, square 1:1, social-media ready."""
                        ...
                        try:
                            ...
                            st.session_state.ph_variants[selected_idx] = updated_variant
            ...
            variant = st.session_state.ph_variants[selected_idx]
            file_ext = (variant.get("format") or "png").lower()
            mime_type = "image/jpeg" if file_ext == "jpg" else "image/png"
            st.download_button(
                "Download",
                data=variant["img"],
                file_name=f"product_hero_variant_{selected_idx + 1}.{file_ext}",
                mime=mime_type,
            ...
            if st.button(
                ...
                st.session_state.img_mode = "Inspire"
                st.success(
