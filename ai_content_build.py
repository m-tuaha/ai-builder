    ...
    with st.form("campaign_form"):
        ...
        if campaign_type == "OTT / Direct Messaging":
            ...
            with st.expander("Copy My Style (Social) — optional"):
                ...
                with colB:
                    st.checkbox("Use this style for Social now", key="use_style_card_social")
                ...
                if build_card_btn:
                    ...
                    try:
                        ...
                        st.success("Style card built and enabled for Social.")
                        st.stop()  # end form submit here (prevents accidental generate on same run)
                    except Exception as e:
                        st.error(f"Failed to build style card: {e}")
                        st.stop()
