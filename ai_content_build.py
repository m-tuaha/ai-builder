...
def add_text_overlay(img_bytes: bytes, text: str, font_size: int, color_hex: str,
                     pos_x_pct: int, pos_y_pct: int, font_path: str | None = None) -> bytes:
    ...
    # Draw text with light outline for readability
...
with tab2:
    ...
    # ---------- QUICK ACTIONS (Pillow) ----------
        ...
        qa = st.selectbox("Choose an action", ["Add Text Overlay", "Resize/Crop (coming next)", "Color Swap (coming next)"], key="qa_choice")
        ...
        if qa == "Add Text Overlay":
            ...
            with colA:
                ...
                color = st.color_picker("Color", value="#FFFFFF", key="qa_color")
            with colB:
                font_size = st.slider("Font size", 12, 160, 48, step=2, key="qa_fontsize")

            st.caption("Position (percentage of image width/height)")
            cx, cy = st.slider("X position %", 0, 100, 50, key="qa_x"), st.slider("Y position %", 0, 100, 50, key="qa_y")

            # ============= LIVE PREVIEW (with outline) =============
            if src_bytes:
                if overlay_text and overlay_text.strip():
                    import io
                    from PIL import Image, ImageDraw, ImageFont

                    _img = Image.open(io.BytesIO(src_bytes)).convert("RGB")
                    _draw = ImageDraw.Draw(_img)

                    try:
                        _font = ImageFont.truetype(font_path, int(font_size)) if font_path else ImageFont.load_default()
                    except Exception:
                        _font = ImageFont.load_default()

                    _x = int(_img.width * (cx / 100.0))
                    _y = int(_img.height * (cy / 100.0))

                    # Outline ring (optional)
                    if add_outline and outline_width > 0:
                        _offsets = [
                        ...
                        for ox, oy in _offsets:
                            _draw.text((_x + ox, _y + oy), overlay_text.strip(), fill=outline_color, font=_font, anchor="mm")

                    # Foreground text
                    _draw.text((_x, _y), overlay_text.strip(), fill=color, font=_font, anchor="mm")
            ...
            if apply:
                ...
                # Quota check (respects exclusion list via your new counter)
                try:
                ...
                try:
                    with st.spinner("Rendering overlay…"):
                        # Try calling your function WITH outline args first (if supported)
                        try:
                            ...
                            # Fallback: function doesn't support outline args
                            out_bytes = add_text_overlay(
                                img_bytes=src_bytes,
                                text=overlay_text.strip(),
                                font_size=int(font_size),
                    ...
                    try:
                        ...
                        # IMPORTANT: meta includes file/url & text for Admin "Details"
                        log_event(
                            uid,
                            "image_generate_done",
                            mode="text_overlay",
                            ...
                            meta={"file": f"{img_id}.png", "url": img_url, "text": overlay_text.strip()},
                        )
                    except Exception as _e:
                    ...
                    st.image(out_bytes, caption="Result", use_container_width=True)
                    st.download_button("📥 Download", data=out_bytes, file_name="text_overlay.png", mime="image/png", use_container_width=True)
