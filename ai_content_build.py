...
if "svg_is_busy" not in st.session_state:
    st.session_state.svg_is_busy = False
if "svg_result_url" not in st.session_state:
    st.session_state.svg_result_url = None
if "svg_result_key" not in st.session_state:
    st.session_state.svg_result_key = None
if "last_vectorize_error" not in st.session_state:
    st.session_state.last_vectorize_error = None
if "qa_svg_upload_file" not in st.session_state:
    st.session_state.qa_svg_upload_file = None
if "qa_svg_result_url" not in st.session_state:
    st.session_state.qa_svg_result_url = None
...
def _extract_first_http_url(value):
    if isinstance(value, str):
        return value if value.startswith("http") else None
    if isinstance(value, list):
        for item in value:
            found = _extract_first_http_url(item)
            if found:
                return found
    if isinstance(value, dict):
        for key in ("vectorized", "svg", "file", "url", "image", "output"):
            if key in value:
                found = _extract_first_http_url(value[key])
                if found:
                    return found
...
def _replicate_vectorize_sync(image_url: str, wait_seconds: int = 60) -> str:
    ...
    headers = {
        "Authorization": f"Bearer {token}",
        "Content-Type": "application/json",
        "Prefer": f"wait={int(wait_seconds)}",
    }
    ...
    try:
        ...
        raise Exception(f"Replicate vectorize request failed: {e}") from e
    ...
    if not svg_url:
        raise Exception("Vectorize succeeded but no SVG URL was returned.")
...
st.set_page_config(page_title="AI Content & Image Generator", layout="wide")
GMS_TEAL = "#E6F9F3"
GMS_GREEN = "#22B573"
GMS_BLUE = "#C7E7FD"
GMS_LAVENDER = "#D5D7FB"
...
def _normalize_mime_from_extension(extension: str) -> str:
...
def _upload_bytes_to_images_bucket(file_bytes: bytes, extension: str, content_type: str) -> tuple[str, str]:
    if not sb:
        raise Exception("Storage client unavailable.")
    ext = extension.lower().lstrip(".") or "bin"
    content_type = content_type or _normalize_mime_from_extension(ext)
    file_id = str(uuid.uuid4())
    path = f"images/{file_id}.{ext}"
    try:
        sb.storage.from_("images").upload(path, file_bytes, {"content-type": content_type})
    except Exception as e:
        raise Exception(f"Failed to upload file to storage: {e}")
    public_url = sb.storage.from_("images").get_public_url(path)
    return file_id, public_url


def _upload_svg_bytes(svg_bytes: bytes) -> tuple[str, str]:
    return _upload_bytes_to_images_bucket(svg_bytes, "svg", "image/svg+xml")


def _upload_image_bytes_for_vectorize(img_bytes: bytes, mime: str | None) -> tuple[str, str]:
    ext = _guess_extension_from_mime(mime)
    content_type = (mime or _normalize_mime_from_extension(ext))
    if content_type == "image/jpg":
        content_type = "image/jpeg"
    return _upload_bytes_to_images_bucket(img_bytes, ext, content_type)


def _vectorize_and_store(image_url: str) -> tuple[str, str, int, int]:
    start_time = time.time()
    svg_remote_url = _replicate_vectorize_sync(image_url)
    try:
        svg_resp = requests.get(svg_remote_url, timeout=60)
        svg_resp.raise_for_status()
    except requests.exceptions.RequestException as e:
        raise Exception(f"Failed to download SVG output: {e}") from e

    svg_bytes = svg_resp.content
    svg_key, supabase_url = _upload_svg_bytes(svg_bytes)
    runtime_ms = int(max(0, (time.time() - start_time) * 1000))
...
def _run_vectorize_flow(image_url: str, source_label: str, user_id: str | None, *, update_quick_actions: bool = False):
    ...
    st.session_state.last_vectorize_error = None
    try:
        svg_key, svg_url, byte_count, runtime_ms = _vectorize_and_store(image_url)
        ...
        try:
            log_event(
                ...
                mode="vectorize",
                status="error",
                meta={"error": str(exc), "source": source_label},
            )
        except Exception:
            pass
        return None, None, 0, 0

    st.session_state.svg_result_key = svg_key
    st.session_state.svg_result_url = svg_url
    ...
    return svg_key, svg_url, byte_count, runtime_ms
...
def _get_excluded_quota_modes():
    ...
    modes.add("vectorize")
    return modes  # e.g., {"text_overlay","resize_crop","vectorize"}
...
with tab2:
    ...
    # ---------- INSPIRE (template-assisted or freeform) ----------
        ...
        input_bytes = None
        input_mime = None
        vectorize_source = None
        vectorize_public_url = None
        ...
        if st.session_state.get("chained_image") and st.session_state.get("edit_mode") == mode:
            ...
            vectorize_source = "generated"
            vectorize_public_url = st.session_state.get("last_image_url")            
            ...
            if uploaded:
                ...
                input_mime = uploaded.type or "image/png"
                vectorize_source = "upload"
        ...
        if input_bytes:
            with st.container():
                ...
                convert_disabled = st.session_state.svg_is_busy
                convert_clicked = st.button(
                    "Convert to SVG (beta)",
                    key="inspire_svg_convert_btn",
                ...
                if convert_clicked:
                    ...
                    source_label = vectorize_source or "upload"
                    click_meta = {
                        "type": "svg",
                        "source": source_label,
                        "provider": "replicate",
                    ...
                    image_url = vectorize_public_url
                    try:
                        if not image_url:
                            _, image_url = _upload_image_bytes_for_vectorize(input_bytes, input_mime)
                    except Exception as exc:
                        st.session_state.last_vectorize_error = str(exc)
                        try:
                            log_event(
                        ...
                        with st.spinner("Converting to SVG…"):
                            _run_vectorize_flow(image_url, source_label, user_id)
                    finally:
                        st.session_state.svg_is_busy = False

                if st.session_state.last_vectorize_error:
                    st.error(f"SVG conversion failed: {st.session_state.last_vectorize_error}")
                elif st.session_state.svg_result_url:
                    st.image(st.session_state.svg_result_url, use_container_width=True)
                    st.link_button(
                        "Download SVG",
                        st.session_state.svg_result_url,
                        key="inspire_svg_download_btn",
                    )
    ...
    # ---------- QUICK ACTIONS (Pillow) ----------
        ...
        qa = st.selectbox(
            "Choose an action",
            ["Add Text Overlay", "Resize/Crop", "Convert to SVG (beta)"],
            key="qa_choice",
        )

        if qa == "Convert to SVG (beta)":
            svg_upload = st.file_uploader(
                "Upload an image",
                type=["png", "jpg", "jpeg", "webp"],
                key="qa_svg_uploader",
            )
            stored_svg = st.session_state.get("qa_svg_upload_file")

            if svg_upload:
                file_bytes = svg_upload.read()
                file_mime = svg_upload.type or "image/png"
                file_hash = hashlib.sha1(file_bytes).hexdigest()
                needs_upload = True
                if stored_svg and stored_svg.get("hash") == file_hash:
                    needs_upload = False
                if needs_upload:
                    try:
                        source_key, source_url = _upload_image_bytes_for_vectorize(file_bytes, file_mime)
                    except Exception as exc:
                        st.session_state.qa_svg_upload_file = None
                        st.session_state.qa_svg_result_url = None
                        st.session_state.last_vectorize_error = str(exc)
                        ...
                        st.session_state.qa_svg_upload_file = {
                            "bytes": file_bytes,
                            "mime": file_mime,
                            "name": svg_upload.name,
                            "url": source_url,
                    ...
                    st.session_state.qa_svg_upload_file = stored_svg

            stored_svg = st.session_state.get("qa_svg_upload_file")
            if stored_svg and stored_svg.get("bytes"):
                st.image(stored_svg["bytes"], caption="Uploaded image", use_container_width=True)
            else:
                st.info("Upload a PNG, JPG, or WebP image to convert it into SVG.")

            convert_disabled = st.session_state.svg_is_busy or not (stored_svg and stored_svg.get("url"))
            convert_clicked = st.button(
                "Convert",
                key="qa_svg_convert_btn",
                disabled=convert_disabled,
            )        
            ...
            if convert_clicked:
                ...
                image_url = stored_svg.get("url") if stored_svg else None
                try:
                    if not image_url:
                        raise Exception("Missing uploaded image URL.")
                    with st.spinner("Converting to SVG…"):
                        _run_vectorize_flow(image_url, "upload", user_id, update_quick_actions=True)
                except Exception as exc:
                    st.session_state.last_vectorize_error = str(exc)
                    try:
                        log_event(
                            ...
                            mode="vectorize",
                            status="error",
                            meta={"error": str(exc), "source": "upload"},
                        )
                    except Exception:
                        pass
                finally:
            ...
            if st.session_state.last_vectorize_error:
                st.error(f"SVG conversion failed: {st.session_state.last_vectorize_error}")
            elif st.session_state.qa_svg_result_url:
                st.image(st.session_state.qa_svg_result_url, use_container_width=True)
                st.link_button(
                    "Download SVG",
...
with tab3:
    ...
    left_col, center_col, right_col = st.columns([1.25, 1.5, 1.0])
