...
def reset_today_quota(user_id: str):
    ...
    rows = (sb.table("events")
            ...
            .execute()).data or []
    for r in rows:
        sb.table("events").delete().eq("id", r["id"]).execute()
    return len(rows)
...
def fetch_events(start_iso: str, end_iso: str, user_id: str | None, mode: str | None):
    if not sb: return []
    q = (sb.table("events")
         .select("id, ts, user_id, action, mode, model, status, image_id, prompt")
...
def purge_older_than_30d():
    ...
    old_imgs = sb.table("images").select("id").lt("created_at", cutoff).execute().data or []
    if old_imgs:
        paths = [f"images/{r['id']}.png" for r in old_imgs]
        try: sb.storage.from_("images").remove(paths)
        except Exception: pass
        for r in old_imgs:
            sb.table("images").delete().eq("id", r["id"]).execute()
    old_events = sb.table("events").select("id").lt("ts", cutoff).execute().data or []
    for r in old_events:
        sb.table("events").delete().eq("id", r["id"]).execute()
    return len(old_imgs), len(old_events)
...
def render_admin():
    ...
    with colf1: start = st.date_input("From", today - dt.timedelta(days=7))
    with colf2: end = st.date_input("To", today + dt.timedelta(days=1))
    users = list_users()
    user_map = {u["username"]: u["id"] for u in users}
    with colf3: user_choice = st.selectbox("User", ["All"] + list(user_map.keys()))
    mode_choice = st.selectbox("Mode", ["All", "text", "create", "inspire", "combine"])
    ...
    if uid:
        ...
        if st.button("Reset today's image quota for this user"):
            ...
            st.success(f"Reset done. Removed {removed} success events.")
    else:
    ...
    if rows:
        ...
        table = []
        for r in rows:
            date_str = r["ts"].replace("T", " ").split(".")[0]
            uname = uname_lookup.get(r["user_id"], "—")
            prompt_disp = (r.get("prompt") or "")[:300]
            res = "Error" if r.get("status")=="error" else ("Image" if r.get("image_id") else "Text")
            table.append({"Date": date_str, "User": uname, "Mode": r.get("mode"), "Prompt": prompt_disp, "Result": res})
    ...
    try:
        imgs = sb.table("images").select("id, url, user_id, created_at, mode").order("created_at", desc=True).limit(30).execute().data or []
    except Exception:
        imgs = []
    if imgs:
        cols = st.columns(3)
        for i, img in enumerate(imgs):
            with cols[i%3]:
                st.image(img["url"], caption=f"{img['mode']} • {img['created_at'][:19]}", use_container_width=True)
                try:
...
with tab1:
    ...
    if generate_btn and prompt:
        ...
        try:
            ...
            try:
                log_event(st.session_state.auth_user["id"], "text_generate_done", mode="text", status="success", prompt=prompt, prompt_len=len(prompt))
            except Exception:
                pass
        ...
        try:
            log_event(st.session_state.auth_user["id"], "text_generate_done", mode="text", status="error", prompt=prompt, prompt_len=len(prompt))
        except Exception:
            pass
            ...
            with st.spinner("🎨 Generating your image..."):
                try:
                    ...
                    try:
                        ...
                        log_event(st.session_state.auth_user["id"], "image_generate_done", mode=_mode_key, status="success", image_id=img_id, prompt=_prompt_for_log, prompt_len=len(_prompt_for_log))
