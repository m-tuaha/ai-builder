...
with tab2:
    ...
    # ---------- INSPIRE (template-assisted or freeform) ----------
        ...
        if templates:
            ...
            for i, tmpl in enumerate(templates):
                ...
                with c:
                    ...
                    # -- Tiny Before→After strip (optional; no captions, no full-size expander)
                    try:
                        _render_preview_pair(tmpl)  # harmless no-op if previews not provided
                    except Exception:
                        pass  # previews are optional; ignore any errors

                    # -- Use Example (prefill the single text area only when clicked)
        ...
        final_prompt = st.text_area(
            ...
            placeholder="Type your own prompt here, or click 'Use Example' on a template above",
            height=120,
