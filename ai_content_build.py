...
def render_admin():
    ...
    try:
        ...
        # Image successes (non-voided)
        _img_ok = (sb.table("events").select("id", count="exact")
                   .eq("action","image_generate_done").eq("status","success")
                   .or_("meta->>voided.is.null,meta->>voided.eq.false")
                   .gte("ts", _start.isoformat()).lt("ts", _end.isoformat()).execute()).count or 0
        # Text successes
        _txt_ok = (sb.table("events").select("id", count="exact")
                   .eq("action","text_generate_done").eq("status","success")
                   .gte("ts", _start.isoformat()).lt("ts", _end.isoformat()).execute()).count or 0
        # Errors
        _errs = (sb.table("events").select("id", count="exact")
        ...
        # Mode split last 7 days (images)
        _week_start = (_now - dt.timedelta(days=7)).isoformat()
        _img_rows = (sb.table("events").select("mode")
                     .eq("action","image_generate_done").eq("status","success")
                     .gte("ts", _week_start).execute()).data or []
        _mode_counts = {}
        for r in _img_rows:
...
if rows:
    ...
    table = []
    for r in rows:
        # Localize ts if possible
        _ts = r.get("ts")
        try:
            _dt = dt.datetime.fromisoformat(_ts.replace("Z","+00:00"))
            _dt = _dt.astimezone(_tz)
            date_str = _dt.strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            date_str = (_ts or "").replace("T"," ").split(".")[0]
        uname = uname_lookup.get(r["user_id"], "—")
        result = "Error" if r.get("status") == "error" else ("Image" if r.get("image_id") else ("Refine" if r.get("action")=="image_refine_prompt" else "Text"))
        meta = r.get("meta") or {}
        if result == "Error":
            ...
            details = _short((meta.get("after") or ""))
        else:
            details = ""
        table.append({
            "Date": date_str,
    ...
    # Optional full detail viewer
    try:
        row_idx = st.number_input("View row #", min_value=0, max_value=max(0,len(rows)-1), value=0)
        st.text_area("Full Prompt", rows[row_idx].get("prompt") or "", height=140)
        _m = rows[row_idx].get("meta") or {}
        st.text_area("Full Details", (_m.get("error") or _m.get("text") or _m.get("after") or _m.get("url") or ""), height=180)
    except Exception:
        pass
else:
