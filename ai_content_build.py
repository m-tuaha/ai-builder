...
# Choose system prompt for text generation
system_prompt = SOCIAL_SYSTEM_PROMPT if channel == "social_post" else TEXT_SYSTEM_PROMPT
...
with tab1:
    ...
    with st.form("campaign_form"):
        ...
        if campaign_type == "OTT / Direct Messaging":
            ...
            max_length = st.number_input("Max Length", min_value=1, max_value=1024, value=600)
            variants = st.number_input("Number of Variants", min_value=1, max_value=3, value=1)
            platform = None  # not used for OTT
        else:
            # --- Social fields (single variant, no placeholders expected) ---
            platform = st.selectbox("Platform", ["linkedin", "facebook", "viva_engage"])
            prompt = st.text_area(
                "Campaign Instruction / Prompt",
                placeholder="Describe the announcement, target audience, key points, and any special instructions."
            )
            language = st.text_input("Language", "en")
            tone = st.text_input("Tone", "professional")
            max_length = st.number_input("Max Length", min_value=100, max_value=3000, value=1000)
            variants = 1  # fixed for social (one-to-many)
        ...
        try:
            ...
            try:
                log_event(
                    ...
                    prompt=prompt,
                    prompt_len=len(prompt),
                    meta={"text": (st.session_state.last_output or {}).get("body", "")}
                )
            except Exception:
                pass
    ...
    if st.session_state.last_variants:
        if len(st.session_state.last_variants) > 1:
            ...
            if "chat_history" in st.session_state and st.session_state.chat_history:
                if len(st.session_state.chat_history) == 3 and st.session_state.chat_history[2]["role"] == "assistant":
            ...
            with st.spinner("🎨 Generating your image..."):
                try:
                    if mode == "Create":
                        ...
                        img_bytes = generate_kontext_max(final, uri)
