...
def generate_nano_banana_replicate(
    ...
    Ignores AR/seed/prompt_upsampling; we only send prompt + image_input[] + output_format.
    """
    token = st.secrets["REPLICATE_API_TOKEN"]
    headers = {
        "Authorization": f"Bearer {token}",
    ...
    ofmt = (output_format or "png").lower()
    payload_input = {"prompt": prompt, "output_format": ofmt if ofmt in {"png", "jpg"} else "png"}
    if image_data_urls:
