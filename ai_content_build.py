    ...
    # ---------- QUICK ACTIONS (Pillow) ----------
        ...
        if qa == "Resize/Crop":
            ...
            # ---------- CUSTOM: use streamlit-cropper (no X/Y anchors) ----------
                ...
                # Safe local import so you don't have to modify imports elsewhere
                try:
                    from streamlit_cropper import st_cropper
                except Exception as _e:
                    st.error("streamlit-cropper is not installed. Add 'streamlit-cropper' to requirements.txt and redeploy.")
                    st.stop()
                ...
                try:
                    ...
                    cropped = st_cropper(
                        ...
                        return_type="image",    # <- FIX
                        box_algorithm="simple",
                        key="rc_cropper"
                    )
                    ...
                    with colw:
                        export_w = st.number_input("Export width (px)", min_value=64, max_value=4096, value=cropped.size[0], key="rc_exp_w")
                    with colh:
                        export_h = st.number_input("Export height (px)", min_value=64, max_value=4096, value=cropped.size[1], key="rc_exp_h")
                    ...
                    if apply_rc_custom:
                        ...
                        try:
                            # Resize to requested export size if different
                            out_img = cropped
                            if (int(export_w), int(export_h)) != cropped.size:
                                out_img = cropped.resize((int(export_w), int(export_h)), Image.LANCZOS)
                            ...
                            log_event(
                                ...
                                meta={
                                    ...
                                    "url": img_url,
                                    "preset": "Custom",
                                    "export_w": int(export_w),
                                    "export_h": int(export_h),
                                },
                            )
                            ...
                            try:
                                log_event(
                                    ...
                                    mode="resize_crop",
                                    status="error",
                                    prompt="Resize/Crop (Custom) failed",
                                    prompt_len=0,
                                    meta={"error": str(e)},
                                )
                            except Exception:
