...
def _resolve_path_or_url(path_or_url: str | None) -> str | None:
    """Accepts http(s)/data URLs or relative file paths next to the app."""
    if not path_or_url:
        return None
...
def _render_preview_pair(tmpl: dict, small_height: int = 110):
    ...
    cap = tmpl.get("caption") or tmpl.get("example")
    if cap:
        st.caption(cap)
        ...
        if templates:
            ...
            for i, tmpl in enumerate(templates):
                ...
                with c:
                    ...
                    # -- Tiny Before→After strip (only if preview images exist)
                    try:
                        _render_preview_pair(tmpl)
                    except Exception as _pe:
                        # Silent: previews are optional
                        pass

                    # -- Use Example (prefill the single text area)
                    if st.button("Use Example", key=f"use_{tmpl['id']}"):
                        ex = (tmpl.get("example") or "").strip()
                        if ex:
                            st.session_state.inspire_prompt = ex

                    # -- Optional: larger pair in an expander (only if provided)
                    fb = _resolve_path_or_url(tmpl.get("full_before"))
                    fa = _resolve_path_or_url(tmpl.get("full_after"))
                    if fb or fa:
                        with st.expander("See larger preview", expanded=False):
                            cL, cR = st.columns(2)
                            with cL:
                                st.caption("Before (large)")
                                if fb: st.image(fb, use_container_width=True)
                            with cR:
                                st.caption("After (large)")
                                if fa: st.image(fa, use_container_width=True)

                    # Keep example text visible, but non-blocking
                    if tmpl.get("example"):
                        st.caption(f"“{tmpl['example']}”")
               
        # single editable text area (freeform or prefilled)
        final_prompt = st.text_area(
            "Prompt for Kontext",
            value=st.session_state.inspire_prompt,
            placeholder="Type your own prompt here, or use a template above",
            height=120,
            key="inspire_text_area"
        )
