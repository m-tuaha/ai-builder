...
# SVG/vectorization defaults
st.session_state.setdefault("svg_opts_colors", 5)
st.session_state.setdefault("svg_opts_smoothness", "medium")
st.session_state.setdefault("svg_opts_bg", "remove")
st.session_state.setdefault("svg_is_busy", False)
st.session_state.setdefault("svg_result_url", None)
st.session_state.setdefault("svg_result_key", "")
st.session_state.setdefault("svg_last_source", None)
st.session_state.setdefault("last_vectorize_error", None)
st.session_state.setdefault("svg_result_bytes", None)
st.session_state.setdefault("svg_result_filename", None)
st.session_state.setdefault("svg_click_logged", {})
...
def _downscale_image(img: Image.Image, max_long_edge: int = 1600) -> Image.Image:
    """Downscale image so the longest edge is <= max_long_edge while preserving aspect."""
    if not img:
        raise ValueError("Image is required for downscaling")
    w, h = img.size
    longest = max(w, h)
...
def _quantize_image(img: Image.Image, n_colors: int):
    """Quantize an RGB image into n_colors and return (palette, indexed_image)."""
    if n_colors < 1:
        raise ValueError("n_colors must be >= 1")
    rgb_img = img.convert("RGB")
    method = getattr(Image, "MAXCOVERAGE", Image.MEDIANCUT)
    try:
        indexed = rgb_img.quantize(colors=n_colors, method=method, kmeans=max(1, n_colors * 2))
    except Exception:
        try:
            indexed = rgb_img.quantize(colors=n_colors, method=Image.MEDIANCUT)
        except Exception:
            indexed = rgb_img.quantize(colors=n_colors)
    palette = indexed.getpalette() or []
    palette_rgb: list[tuple[int, int, int]] = []
    for i in range(n_colors):
        base = i * 3
        if base + 2 < len(palette):
            palette_rgb.append((palette[base], palette[base + 1], palette[base + 2]))
        else:
            palette_rgb.append((0, 0, 0))
    return palette_rgb, indexed


def _trace_regions(indexed_img: Image.Image, target_index: int, alpha_mask: Image.Image | None = None):
    """Return polygons (list of points) for connected regions matching target_index."""
    w, h = indexed_img.size
    pixels = indexed_img.load()
    alpha_px = alpha_mask.load() if alpha_mask else None
    visited = [[False] * w for _ in range(h)]
    regions: list[list[tuple[float, float]]] = []

    def _component_to_polygons(component: set[tuple[int, int]]):
        ...
        while adjacency:
            start = min(adjacency.keys(), key=lambda pt: (pt[1], pt[0]))
            polygon: list[tuple[float, float]] = []
            current = start
            prev = None
            while True:
                polygon.append((float(current[0]), float(current[1])))
                neighbors = adjacency.get(current)
                if not neighbors:
                    break
                if prev is None:
                    nxt = sorted(neighbors, key=lambda pt: (pt[1], pt[0], pt[0]))[0]
                else:
                    candidates = [pt for pt in neighbors if pt != prev]
                    if candidates:
                        candidates.sort(key=lambda pt: (pt[1], pt[0], pt[0]))
                        nxt = candidates[0]
                    else:
                        nxt = prev
                adjacency[current].discard(nxt)
                if not adjacency[current]:
                    del adjacency[current]
                if nxt in adjacency:
                    adjacency[nxt].discard(current)
                    if not adjacency[nxt]:
                        del adjacency[nxt]
    ...
    for y in range(h):
        for x in range(w):
            if visited[y][x]:
                continue
            visited[y][x] = True
            if pixels[x, y] != target_index:
                continue
            if alpha_px is not None and alpha_px[x, y] < 128:
                continue
            ...
            while queue:
                ...
                for nx, ny in ((cx + 1, cy), (cx - 1, cy), (cx, cy + 1), (cx, cy - 1)):
                    if 0 <= nx < w and 0 <= ny < h and not visited[ny][nx]:
                        visited[ny][nx] = True
                        if pixels[nx, ny] != target_index:
                            continue
                        if alpha_px is not None and alpha_px[nx, ny] < 128:
                            continue
                        component.add((nx, ny))
                        queue.append((nx, ny))
            if component:
                regions.extend(_component_to_polygons(component))
    return regions
...
def _rdp(points: list[tuple[float, float]], epsilon: float) -> list[tuple[float, float]]:
    ...
    def _rdp_open(seq: list[tuple[float, float]], eps: float) -> list[tuple[float, float]]:
        if len(seq) < 3:
            return seq[:]
        start = seq[0]
        end = seq[-1]
        ...
        for i in range(1, len(seq) - 1):
            ...
            if dist > max_dist:
                max_dist = dist
                index = i
        if max_dist > eps and index != -1:
            left = _rdp_open(seq[: index + 1], eps)
            right = _rdp_open(seq[index:], eps)
            return left[:-1] + right
        return [start, end]

    closed = points + [points[0]]
    simplified = _rdp_open(closed, epsilon)
    if simplified and simplified[0] == simplified[-1]:
        simplified = simplified[:-1]
    return simplified


def _build_svg_xml(width: int, height: int, shapes, bg_fill: str | None) -> bytes:
    """Build a minimal SVG document from shape definitions."""
    ...
    if bg_fill:
        parts.append(f'<rect width="100%" height="100%" fill="{bg_fill}"/>')
    for shape in shapes:
        pts = shape.get("points") or []
        if len(pts) < 3:
            continue
        fill = shape.get("fill", "#000000").upper()
        commands = []
        for idx, (px, py) in enumerate(pts):
            cmd = "M" if idx == 0 else "L"
            commands.append(f"{cmd}{_fmt(px)},{_fmt(py)}")
        commands.append("Z")
        path_data = " ".join(commands)
        parts.append(f'<path fill="{fill}" d="{path_data}"/>')
    parts.append("</svg>")
    return "".join(parts).encode("utf-8")


def _save_svg_to_storage(svg_bytes: bytes, filename: str) -> str:
    """Persist SVG in the same storage bucket used for images and return the public URL."""
    if not sb:
...
def _vectorize_to_svg(image_bytes: bytes, colors: int, smoothness: str, bg_mode: str) -> bytes:
    """Full vectorization pipeline returning SVG bytes."""
    if not image_bytes:
        raise ValueError("No image data provided")
    with Image.open(io.BytesIO(image_bytes)) as src:
        rgba = src.convert("RGBA")
    downscaled = _downscale_image(rgba)
    rgb_img = downscaled.convert("RGB")
    alpha_mask = downscaled.getchannel("A") if ("A" in downscaled.getbands() and bg_mode == "remove") else None

    palette_rgb, indexed = _quantize_image(rgb_img, colors)

    smooth_map = {"low": 0.75, "medium": 1.5, "high": 3.0}
    epsilon = smooth_map.get((smoothness or "").lower(), 1.5)

    shapes = []
    for idx, rgb in enumerate(palette_rgb):
        polygons = _trace_regions(indexed, idx, alpha_mask=alpha_mask)
        if not polygons:
            continue
        fill_hex = "#{:02x}{:02x}{:02x}".format(*rgb).upper()
        for polygon in polygons:
            simplified = _rdp(polygon, epsilon)
            if len(simplified) < 3:
                continue
            shapes.append({"fill": fill_hex, "points": simplified})

    def _dominant_color_hex(img: Image.Image) -> str:
        try:
            colors = img.getcolors(maxcolors=img.width * img.height)
        except Exception:
            sample = img.resize((min(64, img.width), max(1, int(img.height * min(64, img.width) / max(1, img.width)))), Image.NEAREST)
    ...
    if bg_mode == "keep":
        background_fill = _dominant_color_hex(rgb_img)

    return _build_svg_xml(indexed.width, indexed.height, shapes, background_fill)


def _log_vectorize_panel_open(source: str):
    """Log a single vectorize_click event per source per session."""
    try:
        logged = st.session_state.get("svg_click_logged", {})
        if logged.get(source):
            return
        defaults = {"colors": 5, "smoothness": "medium", "background": "remove"}
        log_event(
            st.session_state.get("auth_user", {}).get("id"),
            "vectorize_click",
            mode="vectorize",
            meta={"type": "svg", "defaults": defaults},
        )
        logged[source] = True
        st.session_state.svg_click_logged = logged
    except Exception:
        pass


def _run_svg_conversion(image_bytes: bytes, source: str, image_id: str | None = None):
    """Execute SVG vectorization, persist result, update UI state, and log outcomes."""
    colors = int(st.session_state.get("svg_opts_colors", 5))
    smoothness = st.session_state.get("svg_opts_smoothness", "medium")
    bg_mode = st.session_state.get("svg_opts_bg", "remove")

    st.session_state.svg_is_busy = True
    st.session_state.svg_result_url = None
    st.session_state.svg_result_bytes = None
    st.session_state.svg_result_filename = None
    st.session_state.last_vectorize_error = None

    started = time.perf_counter()
    try:
        svg_bytes = _vectorize_to_svg(image_bytes, colors=colors, smoothness=smoothness, bg_mode=bg_mode)
        runtime_ms = int((time.perf_counter() - started) * 1000)
        filename = f"{uuid.uuid4()}.svg"
        public_url = _save_svg_to_storage(svg_bytes, filename)

        st.session_state.svg_result_url = public_url
        st.session_state.svg_result_bytes = svg_bytes
        st.session_state.svg_result_filename = filename
        st.session_state.svg_result_key = uuid.uuid4().hex
        st.session_state.svg_last_source = source
        st.session_state.last_vectorize_error = None
        ...
        st.session_state.last_vectorize_error = str(exc)
        st.session_state.svg_last_source = source
        try:
            log_event(
                st.session_state.get("auth_user", {}).get("id"),
                ...
                mode="vectorize",
                image_id=image_id,
                meta={"error": str(exc), "source": source},
            )
        except Exception:
            pass
        raise
    finally:
        st.session_state.svg_is_busy = False


def _get_generated_image_bytes() -> bytes | None:
    """Return bytes for the currently displayed generated image if available."""
    cached = st.session_state.get("generated_image")
    if cached:
        return cached
    url = st.session_state.get("last_image_url")
    if not url:
        return None
    try:
...
with tab2:
    ...
    # ---------- QUICK ACTIONS (Pillow) ----------
        ...
        with st.expander("Convert to SVG (beta)", expanded=False):
            _log_vectorize_panel_open("upload")
            ...
            if svg_upload:
                ...
                st.image(upload_bytes, caption="Uploaded image", use_container_width=True)
            ...
            st.radio(
                ...
                horizontal=True,
                key="svg_opts_smoothness",
            )
            st.radio(
                "Background",
                options=["remove", "keep"],
                format_func=lambda v: "Remove" if v == "remove" else "Keep",
                horizontal=True,
                key="svg_opts_bg",
            )
            ...
            start_upload = st.button(
                "Start Conversion",
                key="svg_start_upload",
                use_container_width=True,
                disabled=st.session_state.get("svg_is_busy", False),
            )
            if start_upload:
                if not upload_bytes:
                    ...
                    try:
                        with st.spinner("Converting to SVG…"):
                            _run_svg_conversion(upload_bytes, source="upload")
                    except Exception:
                        pass

            if st.session_state.get("svg_last_source") == "upload":
                if st.session_state.get("last_vectorize_error"):
                    st.error(f"SVG conversion failed: {st.session_state.last_vectorize_error}")
                elif st.session_state.get("svg_result_url"):
                    st.success("SVG ready!")
                    st.image(
                        st.session_state.svg_result_url,
                        caption="SVG Preview",
                        use_container_width=True,
                        output_format="SVG",
                        clamp=True,
                    ...
                    if st.session_state.get("svg_result_bytes"):
                        st.download_button(
                            "📥 Download SVG",
                            data=st.session_state.svg_result_bytes,
                            file_name=st.session_state.get("svg_result_filename") or "vectorized.svg",
                            mime="image/svg+xml",
                            key=f"svg_download_{st.session_state.get('svg_result_key') or 'upload'}",
                            use_container_width=True,
                        )
                    if st.session_state.get("svg_result_url"):
                        st.link_button(
                            "🔗 Open SVG",
                            st.session_state.svg_result_url,
                            use_container_width=True,
                            key=f"svg_link_upload_{st.session_state.get('svg_result_key') or 'upload'}",
                        )                    
...
# Display results outside the spinner and button logic
    if st.session_state.get("generation_success"):
        ...
        if st.session_state.get("generated_image"):
            ...
            if current_mode == "Inspire":
                _log_vectorize_panel_open("generated")
                with st.container(border=True):
                    st.markdown("#### Convert to SVG (beta)")
                    st.caption("Transform this image into a scalable vector graphic.")
                    st.selectbox(
                        "Max Colors",
                        options=list(range(3, 9)),
                        key="svg_opts_colors",
                    ...
                    start_svg = st.button(
                        "Start Conversion",
                        key="svg_start_generated",
                        use_container_width=True,
                        disabled=st.session_state.get("svg_is_busy", False),
                    )
                    if start_svg:
                        image_bytes = _get_generated_image_bytes()
                        if not image_bytes:
                            st.error("Source image unavailable for vectorization.")
                        else:
                            try:
                                with st.spinner("Converting to SVG…"):
                                    _run_svg_conversion(
                                        image_bytes,
                                        source="generated",
                                        image_id=st.session_state.get("last_image_id"),
                                    )
                            except Exception:
                                pass

                    if st.session_state.get("svg_last_source") == "generated":
                        if st.session_state.get("last_vectorize_error"):
                            st.error(f"SVG conversion failed: {st.session_state.last_vectorize_error}")
                        elif st.session_state.get("svg_result_url"):
                            st.success("SVG ready!")
                            ...
                            if st.session_state.get("svg_result_bytes"):
                                st.download_button(
                                    ...
                                    file_name=st.session_state.get("svg_result_filename") or "vectorized.svg",
                                    mime="image/svg+xml",
                                    key=f"svg_download_{st.session_state.get('svg_result_key') or 'generated'}",
                                    use_container_width=True,
                                )
                            if st.session_state.get("svg_result_url"):
                                st.link_button(
                                    ...
                                    st.session_state.svg_result_url,
                                    use_container_width=True,
                                    key=f"svg_link_generated_{st.session_state.get('svg_result_key') or 'generated'}",
                                )
