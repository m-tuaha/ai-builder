...
def color_swap_image(
    ...
    src_hex: str,
    dst_hex: str,
    tolerance_deg: int = 25,    # hue match window in degrees
    strength: float = 0.85,     # 0..1, how hard to push toward dst hue
    region_bbox: tuple[int,int,int,int] | None = None,  # (left, top, right, bottom) in px
    sat_floor: float = 0.08,    # ignore very gray pixels
    val_floor: float = 0.08,    # ignore very dark pixels
) -> bytes:
    """
    Recolors pixels whose hue ≈ src_hex toward dst_hex in the specified region (or whole image).
    Keeps S & V mostly intact; only shifts H (with 'strength').
    Returns PNG bytes.
    """
    base = Image.open(io.BytesIO(img_bytes)).convert("RGBA")
    work_box = region_bbox or (0, 0, base.width, base.height)

    # Parse colors → hsv (0..1)
    def hex_to_hsv(hx: str):
        r, g, b = _hex_to_rgb(hx)
        h, s, v = colorsys.rgb_to_hsv(r/255.0, g/255.0, b/255.0)
        return h, s, v
    src_h, _, _ = hex_to_hsv(src_hex)
    dst_h, _, _ = hex_to_hsv(dst_hex)
    tol = max(0.0, min(180.0, float(tolerance_deg))) / 360.0
    k = max(0.0, min(1.0, float(strength)))

    # Work on a copy layer
    layer = base.crop(work_box).convert("RGBA")
    rgb = layer.convert("RGB")
    a = layer.split()[-1]  # preserve original alpha

    pixels = list(rgb.getdata())
    new_pixels = []
    for (r, g, b) in pixels:
        hr, sg, vb = r/255.0, g/255.0, b/255.0
        h, s, v = colorsys.rgb_to_hsv(hr, sg, vb)
        ...
        if dh <= tol:
            # shift hue toward dst
            h_new = (1.0 - k) * h + k * dst_h
            rr, gg, bb = colorsys.hsv_to_rgb(h_new, s, v)
            new_pixels.append((int(rr*255), int(gg*255), int(bb*255)))
        else:
            new_pixels.append((r, g, b))

    out_rgb = Image.new("RGB", rgb.size)
    out_rgb.putdata(new_pixels)
    out_rgba = Image.merge("RGBA", (*out_rgb.split(), a))

    # paste recolored region back
    out_img = base.copy()
        ...
        if qa == "Color Swap":
            if not src_bytes:
                ...
                if use_region:
                    ...
                    st.caption("Drag the rectangle; live crop preview on the right.")
                    cropped_img = st_cropper(
                        pil_src,
                        realtime_update=True,
                        box_color="#00483B",   # your teal border
                        aspect_ratio=None,
                        return_type="image",
                        key="cs_cropper"
                    )
                    # compute bbox relative to original if cropper returned an image
                    # We re-crop in JS side, so infer bbox by matching size — easiest is to let user 'Apply' with cropped preview
                    # Instead, we’ll ask the cropper for the box by switching to 'both' briefly:
                    _both = st_cropper(
                        pil_src,
                    ...
                    if isinstance(_both, tuple):
                        _, crop_box = _both  # (PIL.Image, (left, top, right, bottom))
                        left, top, right, bottom = map(int, crop_box)
                        bbox = (max(0,left), max(0,top), min(pil_src.width,right), min(pil_src.height,bottom))
                        st.caption(f"Region: {bbox[2]-bbox[0]}×{bbox[3]-bbox[1]} px")
                        st.image(cropped_img, caption="Region preview", use_container_width=True)
