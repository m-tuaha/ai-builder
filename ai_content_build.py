...
def _render_preview_pair_fixed(tmpl: dict):
    """Render fixed-size before/after thumbs from flat JSON keys."""
    # Support flat keys used in inspire_templates.json
    b = _resolve_path_or_url(tmpl.get("preview_before"))
    a = _resolve_path_or_url(tmpl.get("preview_after"))

    # (Also supports nested {"preview": {"before": "...", "after": "..."}} if present)
    if not (b and a):
...
st.markdown("""
    <style>
      /* INSPIRE template thumbnails: fixed box + crop */
      ...
      .tmpl-thumb {
        width: 240px;          /* set the preview width you want */
        height: 160px;         /* set the preview height you want */
        border-radius: 12px;
        object-fit: cover;     /* crop to fit box, keeps grid tidy */
