...
def purge_older_than_30d():
    """Delete images+rows older than 30 days, and delete events older than 30 days."""
    if not sb: return 0, 0
    cutoff = (dt.datetime.utcnow() - dt.timedelta(days=30)).isoformat()
    # Images (delete storage + rows)
    imgs = (
        sb.table("images").select("id").lt("created_at", cutoff).execute().data or []
    )
    removed_imgs = 0
    for r in imgs:
        try:
            sb.storage.from_("images").remove([f"images/{r['id']}.png"])
        except Exception:
            pass
        try:
    ...
    # Events
    try:
        res = sb.table("events").delete().lt("ts", cutoff).execute()
        removed_events = getattr(res, "count", None) or 0
    except Exception:
        removed_events = 0
    return removed_imgs, removed_events
